...
入口在地图预览页右侧「显示面板 → 地图编辑」，**只在 2D 视图可用**（多边形是画在栅格图上的，3D 点云里没有对应的作图平面）。左键加顶点、右键撤销，至少 3 点才能闭合。
...
的是同一个 0.25m，但**换算方式必须逐字一致**，不然带子正中间会被吃掉。

踩过一次：`clear_trajectory` 是 `int(round(0.25/0.1))` = **2px 方核**，规划器是
...
`tools/check_trajectory_clearance.py --all` 就是验这条不变式的，改动任何一边之后跑一遍。
修之前 3 张 0.1m/格的图分别掉 4 / 36 / 98 个轨迹格（`save_map_small_1` 的轨迹还被切成
11 个互不连通的块），修之后全部 100%。**注意这不是改代码就生效的**，得重新预处理一遍
地图才会体现在 `map_2d.pgm` 上。
...
- **路线数据在 `data/routes/` 下，被 `.gitignore` 排除。** 这是用户自己创作的数据，
  ...
  导出功能。
- **地图编辑的多边形栅格化经过单测，但整套交互未经真人浏览器验证。** 栅格化（矩形/三角形/凹多边形/自相交/图外裁剪/世界坐标闭环）和"编辑真的改变规划结果"都用脚本验过；在图上点选画多边形这套手势没有在真实浏览器里点过。
