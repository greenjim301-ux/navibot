...
- **`detect_structure` 的悬空障碍净空阈值（`--map2d-structure-ground-clearance`，默认 0.6m）没有拿真机验证过。** 判据是"候选障碍格子最低支撑层高度 − 最近建图轨迹点高度"，超过这个值就当狗能从下面钻过去、改判回不占据——0.6m 是凭经验估的机器狗净空高度，偏保守会漏判本该能钻过去的悬空障碍（比如低矮的桌子/置物架仍标成硬挡），偏激进则可能把净空不够的悬空结构误判成"能钻过去"，两种情况都要在实机上试跑几张有悬空障碍的地图才能确认。
