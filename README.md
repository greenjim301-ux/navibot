...
| `GLOBAL_PLANNER_TRAJECTORY_BEATS_INFLATION` | 1 | 轨迹格（+1 圈）**不被膨胀吃掉** |
...
| 鼓包预算 `GLOBAL_PLANNER_MAX_BULGE_M` | 0.28m | 五次曲线允许偏离直线弦多少 |
| 硬上限 `GLOBAL_PLANNER_MAX_WAYPOINT_SPACING_M` | 4.0m | 夹角名义为 0 时也不敢放任的段长 |
...
上面那张表整张可以用一个式子概括(9 个点最大偏差 0.017m):

```
鼓包 ≈ 0.4 * 段长 * sin(入口夹角 α)
```

关键是 **`sin α` 那一项**。原来的实现不看夹角,无条件把所有段切到 1.0m 以内,等于把
"45° 拐弯所需的密度"铺满了整条路线。但直线上 α=0,鼓包本来就是 0,那里切得再密也买不到
任何安全性——**而航点密是有实打实代价的**:轮足机器狗每到一个航点都要减速进
`waypoint_arrival_radius_`(0.3m)再重规划下一段,一路走走停停上不了速度,实机上表现为
全程用脚走、不切轮子(用户实测)。

所以现在按鼓包预算切,而且**不对称**:拐角之后只有第一小段是斜着切进来的,把它压到
`预算 / (0.4 * sin α)` 就够了;它之后插出来的点都落在同一条弦上,转角是 0、鼓包归零,
只受硬上限约束。

| | 改动前 | 现在 |
|---|---|---|
| 30m 笔直一段 | 31 点 | 10 点 |
| 30m 中间折 45° | 38 点 | 13 点 |
| `save_map_small_1` 那条 270m 路线 | 368 点 | 200 点 |
| 同上,最大预测鼓包 | 0.27m | 0.28m |

硬上限 4.0m 的来历:狗是按 `waypoint_arrival_radius_`(0.3m)算"到达"的,加上跟踪误差,
下一段的实际切入角永远不会真的是 0。按残留 10° 估,`0.4*L*sin(10°)=0.069*L`,`L=4.0`
时鼓包 0.28m,跟预算对齐。**这个 10° 是估的,不是实测**——真机上要是还嫌点密/还是不切
轮子,就调 `NAVIBOT_GLOBAL_PLANNER_MAX_WAYPOINT_SPACING_M`。

坡上不受影响:`_prune_path`/`_enforce_min_spacing` 拿 `ground_z` + `max_climb` 卡着,
爬升大的地方本来就合并不起来,长段只会出现在平地上。
