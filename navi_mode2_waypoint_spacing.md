...
状态: **第 4 节的方案已于 2026-09-11 实现(commit `4183847`), 其余仍是调研笔记。**
起因是"global_planner 规划出来的路径点有的太靠近"。
...
本项目侧涉及的是 `backend/app/global_planner.py` 的 `_prune_path`(:341)
和 `_enforce_min_spacing`(:395)。
...
`_prune_path`(`global_planner.py:341`)的贪心 LOS 循环, 内层一旦 `break`
...
## 4. 改法(已实现, 2026-09-11, commit `4183847`)
...
**D. 明确不做: 补最大间距 / 长段插点。** ✅ 没做。之前想过为了让间距均匀而在
长直线段上插点, 按第 0(a) 节这是错的 —— mode 2 没有跨途经点的 min-snap, 均匀
间距没有收益; 而插点会白白截短前瞻, 是净损失。长段超过 4m 时 planner 自己会在
