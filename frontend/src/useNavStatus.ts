...
export function useNavStatus() {
  ...
  //
  // 曾经试过把这两个话题(以及雷达点云)改成二进制 WS 帧 + Float32Array 直接
  // 视图到收到的 ArrayBuffer 上, 打开膨胀地图后浏览器标签页内存会持续上涨、
  // 用一阵子就卡死, 没能在没有真实浏览器环境的情况下把根因排查清楚, 于是整体
  // 回退回这个验证过没问题的 JSON 版本——如果以后要重新尝试二进制协议, 记得
  // 先在真实浏览器里挂着跑一段时间盯内存, 不要只凭静态代码审查断定没问题。
