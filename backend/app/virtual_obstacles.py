...
def _region_cells(points_xy: List[Tuple[float, float]], step: float) -> np.ndarray:
    ...
    shell = _shell(_rasterize_polygon(poly_rc, height, width))
...
def build_points(map_name: str, regions: List[MapEditRegion], delta: Optional[float]) -> np.ndarray:
    """把 regions 里 enabled 的禁行区变成世界系点云 (N, 3) float32。
    ...
    for _ in range(8):  # 点数超标就整体变粗, 不截断(截断会在墙上留洞)
        ...
        for region in blocked:
            xy = _region_cells([(p.x, p.y) for p in region.points], step)
            if len(xy) == 0:
                continue
            ground = _ground_z(map_name, xy)
            if ground is None:
            ...
            block = np.empty((len(xy) * n_layer, 3), dtype=np.float64)
            block[:, 0] = np.repeat(xy[:, 0], n_layer)
            block[:, 1] = np.repeat(xy[:, 1], n_layer)
