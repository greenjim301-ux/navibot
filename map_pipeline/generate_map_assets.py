...
# 的说明) —— 小地图(室内房间尺度)整图一份预览的精度就够用, 强行分片不划算。
#
# 格边长和每格预算是配套调的, 单看一个没意义:
#
# 1) 只有超预算的格子才降采样, 没超的原样导出。实测这批图的格子点数是**极度
#    两极**的 —— 30m 格时中位数才几千点, 但少数几个密集格占了 90~98% 的点,
#    并且超预算好几倍。也就是说"分片会损失细节"只发生在那几个格子上。
# 2) 30m + 15 万时那几个格子被压得很狠: large 最大格 110.5 万点 -> 体素
#    0.177m, 而原始最近邻间距中位数只有 0.032m —— 粗了 5 倍多。
# 3) 改成 15m + 30 万之后, large 只剩 2 个格子要降(且只到 0.118m),
#    save_map_small_1 一个都不用降, 最大分片 4.5MB。基本等于无损。
#
# **预算不能删**, 它是"每个请求的成本上界", 不是平均画质调节器: 前端那套分片
# 加载没有优先级队列/取消/卸载迟滞(见 PointCloudView 的 refreshTiles), 一个
# 16MB 的分片进来就是卡一下, 边界抖动还会反复付这个代价。而且密度完全由数据
# 决定 —— 实测 house(室内)最密处 6330 点/m², large(室外)3042 点/m², 差一个
# 数量级; 缩小格子也**不单调**降低最坏格: house 从 10m 缩到 7.5m, 最大格反而
# 从 63.3 万涨到 71.2 万(密度热点相对网格原点的位置变了)。想靠格边长做到
# "永远不用降采样", house 得切到 2m —— 46m 的图 199 个格子, 一个视野几百个
# 请求, 流式加载就毁了。
TILE_EXTENT_THRESHOLD_M = 150.0
TILE_SIZE_M = 15.0
TILE_POINT_BUDGET = 300_000
